import {
  Input,
  InputNumber,
  List,
  Upload,
  UploadFile,
  UploadProps,
} from "antd";
import { useState } from "react";
import { useTranslation } from "react-i18next";
import { ImageInteraction } from "./ImageInteraction";
import { useUpdateChapterFormState } from "./useUpdateChapterFormState";
import { PlusOutlined } from "@ant-design/icons";

interface UpdateChapterFormProps {
  chapter: Chapter;
}

export const UpdateChapterForm = ({ chapter }: UpdateChapterFormProps) => {
  const { t } = useTranslation();
  const {
    chapterName,
    setChapterName,
    chapterPosition,
    setChapterPosition,
    chapterImagesForView,
    addImageIdToListNeedDelete,
    setNewUploadFiles,
    newUploadFiles,
  } = useUpdateChapterFormState(chapter);

  const handleChange: UploadProps["onChange"] = ({ fileList: newFileList }) => {
    setNewUploadFiles(newFileList);
  };

  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-2 w-[100%]">
        <label htmlFor="chapterName">
          {t("chapterName", { ns: "chapter" })}
        </label>
        <Input
          id="chapterName"
          size="large"
          placeholder={t("chapterName", { ns: "chapter" })}
          value={chapterName}
          onChange={(event) => {
            setChapterName(event.target.value);
          }}
        />
      </div>
      <div className="flex flex-col gap-2 w-[100%]">
        <label htmlFor="chapterName">Thứ tự</label>
        <InputNumber
          size="large"
          value={chapterPosition}
          onChange={(newPosition) => {
            if (newPosition) {
              setChapterPosition(newPosition);
            }
          }}
          changeOnWheel
        />
      </div>
      <div className="flex flex-col gap-2 w-[100%]">
        <div>Danh sách ảnh</div>
        <List
          grid={{ gutter: 3, xs: 3, sm: 5, md: 6, lg: 6, xl: 10, xxl: 10 }}
          dataSource={chapterImagesForView}
          renderItem={(chapterImage) => (
            <List.Item>
              <ImageInteraction
                handleDeleleImage={addImageIdToListNeedDelete}
                chapterImage={chapterImage}
              />
            </List.Item>
          )}
        />
        <Upload
          listType="picture-card"
          fileList={newUploadFiles}
          onPreview={() => false}
          onChange={handleChange}
          multiple
          beforeUpload={() => false}
        >
          <button style={{ border: 0, background: "none" }} type="button">
            <PlusOutlined />
            <div style={{ marginTop: 8 }}>Upload</div>
          </button>
        </Upload>
      </div>
    </div>
  );
};
